...
- **Map search** (`src/lib/leadgen/places.ts`): Places API **(New)** Text Search biased to a real lat/lng + radius, then a Place Details call per result to get its website (Places doesn't return a website in the base search response). Coordinates come from `src/lib/leadgen/searchTerms.ts`'s `UK_REGIONS` — a maintained list of real UK holiday-let hotspots grouped by region, not an algorithmically generated grid. Needs `GOOGLE_PLACES_API_KEY` and a billing-enabled Google Cloud project.
  - **New, not legacy, deliberately.** The old `maps.googleapis.com/maps/api/place/*` endpoints can no longer be enabled on a new Google Cloud project (Google restricted that in 2025) and are a frozen product heading for eventual shutdown — building fresh against them now would mean building on borrowed time. `places.googleapis.com/v1/places:searchText` and `.../places/{id}` are the current, actively-developed replacements.
  - **Real pricing (checked 31 July 2026, the old "$200/month credit" model was retired by Google in March 2025):** Text Search bills on the Pro SKU because `displayName` is requested (5,000 free/month, then $32/1,000). Place Details bills on the Enterprise SKU because `websiteUri` is requested (1,000 free/month, then $20/1,000) — both fields are the whole point of this channel, there's no cheaper field combination that still finds a business's website.
  - **Hitting a real £0 guarantee, not just "should stay free":** Morgan was explicit this must never cost money, and this app has no way to know the exact date the Cloud Billing account's free-tier cycle resets (it isn't necessarily the calendar month). `src/lib/leadgen/usage.ts` tracks Text Search and Place Details calls per calendar month (`LeadPlacesMonthlyUsage`) and hard-stops each one BELOW the real free allowance — 4,000/5,000 for Text Search, 800/1,000 for Place Details — as a deliberate safety margin. Verified against a real database: simulated 850 attempted Place Details calls against the 800 cap, confirmed it stops at exactly 800 every time. When a cap is hit mid-search, the run stops early and says so in the status stream rather than silently returning fewer results.
